...
# Run MCMC
mcmc.samples.all<- nimbleMCMC(code = model,constants = constants,data = data,niter = 100000, nchains = 3, thin=6, nburnin = 10000, summary = FALSE, monitors=c('r1','r2','chp'),WAIC=FALSE,samplesAsCodaMCMC=TRUE,inits=initsFunction,setSeed=c(1,2,3))

gelman.diag(mcmc.samples.all)$psrf[1:3,]

# Store output
save(mcmc.samples.all,file=here('R_image_files','mcmc_samples_all.RData'))
