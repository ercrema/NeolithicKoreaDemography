...
# Thin Dates per Bin
thinned.dates = caldates[thinDates(ages=coastal.koreaC14$c14age,  errors=coastal.koreaC14$c14error, bins=bins, size=1, thresh=1,seed=123,method='splitsample')]
...
# Run MCMC
mcmc.samples.coastal<- nimbleMCMC(code = model,constants = constants,data = data,niter = 100000, nchains = 3, thin=6, nburnin = 10000, summary = FALSE, monitors=c('r1','r2','chp'),WAIC=FALSE,samplesAsCodaMCMC=TRUE,inits=initsFunction,setSeed=c(1,2,3))

gelman.diag(mcmc.samples.coastal)$psrf[1:3,]

# Store output
save(mcmc.samples.coastal,file=here('R_image_files','mcmc_samples_coastal.RData'))
